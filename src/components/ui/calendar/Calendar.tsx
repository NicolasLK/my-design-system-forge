...
import type { IDateRange } from "../date-range-picker/DateRangePicker"
import "./calendar.css"
...
export const Calendar = ({
     ...
     */
    const isMonthControlled = externalMonth !== undefined && setExternalMonth !== undefined;

    const today = useMemo(() => new Date(), []);

    const [internalMonth, setInternalMonth] = useState(externalMonth || selected || today)
            ...
            >
                ...
                <div className="calendar-grid">
                    ...
                    {Array.from({ length: firstDayIndex }).map((_, i) => (
                        <div key={`empty-start-${i}`} className="calendar-empty" />
                    ))}

                    {/* Dias do mês */}
                    {Array.from({ length: daysInMonth }).map((_, i) => {
                        const day = i + 1;
                        const dateToCheck = new Date(year, currentMonth.getMonth(), day);
                        ...
                        const isSelected = selectedTime === dateToCheck.getTime();
                    ...
                    {Array.from({ length: remainingCells }).map((_, i) => (
                        <div key={`empty-end-${i}`} className="calendar-empty" />
                    ))}
                </div>
            </div>
        </>
